...
#				./chia_plot -p $pkey -f $fkey -t ~/chia-p/ -2 ~/chia-p/ -r $(nproc) -u 6 -n 1 >> ~/chia-log/chia.log
				./chia_plot -c $ckey -f $fkey -t ~/chia-p/ -2 ~/chia-p/ -r $(nproc) -u 6 -n 1 >> ~/chia-log/chia.log
				cd ~/chia-p
				rm *.tmp &> /dev/null
